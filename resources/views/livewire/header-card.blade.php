                ...
                @unless (count($headers) == 0)
                    ...
                    @foreach ($this->headers as $header)
                        <div class="owl-carousel-item position-relative">
                            ...
                            <img wire:model.live="headerID" class="img-fluid z-0" src="{{ asset('storage/' . $header->image) }}"
                                alt="{{ $header->name }}">

                                <div class="owl-carousel-text">
                                    <h3 class="display-1 text-white mb-0">{{ $header->name }}</h3>
                                </div>
                    ...
                    @endforeach
                @else
                    <p class="text-black-text-lg text-center italic">Headers Are Not Available At The Moment.</p>
                @endunless
            </div>
